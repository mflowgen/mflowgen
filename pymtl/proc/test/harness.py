...
class TestHarness (Model):
  ...
  def __init__( s, ProcModel, XcelModel, dump_vcd,
    ...
    s.proc   = ProcModel     ()
    s.xcel   = Xcel          ()
    s.mem    = TestMemory    ( MemMsg4B(), 3, mem_stall_prob, mem_latency )
    ...
    # for this lab just tie go bit to high

    s.connect( s.proc.ctrlreg_req.val,  1          )
    s.connect( s.proc.ctrlreg_req.msg,  0b1000000000000000000000000000000000001 )
    s.connect( s.proc.ctrlreg_resp.rdy, 1          )
...
def run_test( ProcModel, XcelModel, gen_test,
  ...
  model = TestHarness( ProcModel, XcelModel, dump_vcd,
                       src_delay, sink_delay,
