...
from CalcShamtPRTL import CalcShamtPRTL
...
class IntMulAltDpathRTL( Model ):
  ...
  def __init__( s ):
    ...
    s.b_zero_cmp = m = ZeroComparator(32)
    s.connect_pairs(
      m.in_, s.b_reg.out,
      m.out, s.is_b_zero,
    )
    ...
    s.rshifter = m = RightLogicalShifter(32,4)
    s.connect_pairs(
      m.in_,   s.b_reg.out,
    ...
    s.lshifter = m = LeftLogicalShifter(32,4)
    s.connect_pairs(
      m.in_,   s.a_reg.out,
      m.shamt, s.calc_shamt.out,
...
class IntMulAltPRTL( Model ):
  ...
  def __init__( s ):
    ...
    s.dpath = IntMulAltDpathRTL()
    s.ctrl  = IntMulAltCtrlRTL()
