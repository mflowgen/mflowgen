...
class TestHarness( Model ):
  ...
  def __init__( s, src_msgs_A, sink_msgs_A, src_msgs_B, sink_msgs_B,
                src_msgs_C, sink_msgs_C,
                src_delay, sink_delay, HostArbMemDummyprocModel,
    ...
    s.src_c  = TestSource( 32,           src_msgs_C,  src_delay  )
    s.dut    = HostArbMemDummyprocModel
    s.sink_a = TestSink  ( MemRespMsg4B, sink_msgs_A, sink_delay )
    s.sink_b = TestSink  ( MemRespMsg4B, sink_msgs_B, sink_delay )
    s.sink_c = TestSink  ( 32,           sink_msgs_C, sink_delay )
    ...
    s.connect( s.src_c.out,  s.dut.host2proc )
    s.connect( s.sink_c.in_, s.dut.proc2host )

  def done( s ):
    return s.src_a.done and s.sink_a.done \
       and s.src_b.done and s.sink_b.done \
       and s.src_c.done and s.sink_c.done
