...
# harness.py
#=========================================================================
# note: this harness only work for top_asic project
...
from top.ProcSram            import ProcSram
...
class TestHarness( Model ):
  ...
  def load_memory( self, mem_image ):
    ...
    for section in sections:
      ...
      if not ( section.name == ".mngr2proc" or section.name == ".proc2mngr" ):
        ...
        # gate-level simulation may fail due to uninitialized memory
        # just add some nops at the end of the .text section
        if section.name == ".text":
          for i in xrange(0,400,4):
            addr = section.addr + len(section.data) + i
            bits = Bits( 32, 0 )
            self.host_src.src.msgs.append  ( MemReqMsg4B.mk_msg ( type_=1, opaque=0, addr=addr, len_=0, data=bits ) )
            self.host_sink.sink.msgs.append( MemRespMsg4B.mk_msg( type_=1, opaque=0, len_=0, data=0 ) )
  ...
  # load_memory_msg
  #-----------------------------------------------------------------------
  # Given a list of MemMsg, load it to a TestSource, to be written to
  # the memory

  def load_memory_msgs( self, msgs ):

    req_msgs  = msgs[::2]
    resp_msgs = msgs[1::2]

    for msg in req_msgs:
      self.host_src.src.msgs.append( msg )
    for msg in resp_msgs:
      self.host_sink.sink.msgs.append( msg )
...
def run_test( dut, gen_test, src_delay, sink_delay, dump_vcd, test_verilog, max_cycles, gen_ref ):
