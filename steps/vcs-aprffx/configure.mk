...
define ascii.vcs-aprffx
	@echo -e $(echo_green)
	@echo '#--------------------------------------------------------------------------------'
	@echo '# VCS APR-FF Sim -- Full X'
	@echo '#--------------------------------------------------------------------------------'
	@echo -e $(echo_nocolor)
endef
...
vcs_aprffx_simv = ./$(handoff_dir.vcs-aprffx-build)/simv
...
#
# - "VCS-APRFFX.mdu.basic_0x0" : Test category "mdu" for test case "basic_0x0"
#

# vcs_aprffx_generate_test_cases
#
# - $(1): test category
# - $(2): test name

define vcs_aprffx_generate_test_cases

$$(logs_dir.vcs-aprffx)/run-$(1)-$(2).log: $$(dependencies.vcs-aprffx)
	@mkdir -p $$(logs_dir.vcs-aprffx)
	@touch $$@.start
	@echo '--------------------------------------------------------------------------------'
	@echo $$@
	@echo '--------------------------------------------------------------------------------'
	$$(vcs_aprffx_simv) $$(vcs_run_options) +test=$(2) 2>&1 | tee $$@

# Create alias target to help run an individual test

VCS_APRFFX.$(1).$(2): $$(logs_dir.vcs-aprffx)/run-$(1)-$(2).log
...
VCS_APRFFX.$(1).$(2).print:
	@echo "$$(vcs_aprffx_simv) $$(vcs_run_options) +test=$(2)"
...
$(foreach category, $(test_categories), \
  $(foreach test, $(tests.$(category)), \
    $(eval $(call vcs_aprffx_generate_test_cases,$(category),$(test)))))
...
#
# - "vcs-aprffx.mdu" : Run all "mdu" test cases
#

# vcs_aprffx_generate_categories
#
# - $(1): test category

define vcs_aprffx_generate_categories
vcs-aprffx.$(1): $$(VCS_APRFFX_$(1)_ALL)
endef

$(foreach category, $(test_categories), \
  $(eval $(call vcs_aprffx_generate_categories,$(category))))

# Summary txt file for the top-level target

vcs_aprffx_summary_txt = $(results_dir.vcs-aprffx)/summary.txt
...
extra_dependencies.vcs-aprffx = \
	$(foreach x, $(test_categories),vcs-aprffx.$x)
...
ifeq (x"$(extra_dependencies.vcs-aprffx)",x"")
define commands.vcs-aprffx
...
define commands.vcs-aprffx
	@mkdir -p $(results_dir.vcs-aprffx)
	@grep -r BRG $(logs_dir.vcs-aprffx) > $(vcs_aprffx_summary_txt)
	@cat $(vcs_aprffx_summary_txt)
	@echo
	@( echo -n "Total passing   : " ; grep -i pass $(vcs_aprffx_summary_txt) | wc -l; \
	   echo -n "Total num tests : " ; wc -l < $(vcs_aprffx_summary_txt) )
endef
endif
...
clean-vcs-aprffx:
	rm -rf ./$(VPATH)/vcs-aprffx
	rm -rf ./$(logs_dir.vcs-aprffx)
	rm -rf ./$(results_dir.vcs-aprffx)
#  rm -rf ./$(collect_dir.vcs-aprffx) # don't clean the YAML file
	rm -rf ./$(handoff_dir.vcs-aprffx)

clean-aprffx: clean-vcs-aprffx clean-vcs-aprffx-build
