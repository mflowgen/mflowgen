...
bins = []
for f in files:
  if os.path.isfile(bin_dir+f) and ('Makefile' not in f) and ('.' not in f):
    bins += [f]
...
for b in bins:
  ...
  with open('vmh/'+b+'.vmh','w') as file_out:
    with open(x,'rb') as file_obj:
      ...
      for section in sections:
        ...
        file_out.write( "@"+format(section.addr,'x')+"\n" )

        for x in section.data:
          file_out.write( format(ord(x),'02x')+"\n" )
